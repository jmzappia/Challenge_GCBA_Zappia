...
{
    /// <summary>
    /// Lock-OpenXmlDocument cmdlet
    /// </summary>
    [Cmdlet(VerbsCommon.Lock, "OpenXmlDocument", SupportsShouldProcess = true)]
    public class LockOpenXmlDocument : OpenXmlCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                foreach (var document in AllDocuments("Lock-OpenXmlDocument", FileAccess.ReadWrite))
                {
                    try
                    {
                        document.InnerContent.Lock();
                        document.FlushParts();
                        if (passThru)
