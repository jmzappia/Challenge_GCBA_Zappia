...
namespace OpenXml.PowerTools.Commands
{
    ...
    public class SetOpenXmlWatermarkCmdlet : OpenXmlCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                foreach (var document in AllDocuments("Set-OpenXmlWatermark", FileAccess.ReadWrite))
                {
                    try
                    {
                        document.InsertWatermark(watermarkText, diagonalOrientation);
                        document.FlushParts();
                        if (passThru)
