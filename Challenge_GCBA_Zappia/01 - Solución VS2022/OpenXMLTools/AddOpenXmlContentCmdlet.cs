...
{
    /// <summary>
    /// Add-OpenXmlContent cmdlet
    /// </summary>
    [Cmdlet(VerbsCommon.Add, "OpenXmlContent", SupportsShouldProcess = true)]
    public class AddOpenXmlContentCmdlet : OpenXmlCmdlet
    {
        ...
        /// PartPath parameter
        /// </summary>
        [Parameter(Position = 1, Mandatory = true, HelpMessage = "Custom Xml part path")]
        ...
        /// Content parameter
        /// </summary>
        [Parameter(Position = 3, Mandatory = true, HelpMessage = "Xml to insert")]
        [ValidateNotNullOrEmpty]
        ...
        protected override void ProcessRecord()
        {
            try
            {
                foreach (var document in AllDocuments("Add-OpenXmlContent", FileAccess.ReadWrite))
                {
                    try
                    {
                        foreach (string xmlPartPath in xmlPartPaths)
                            document.InnerContent.InsertXml(xmlPartPath, xpathInsertionPoint, xmlContent);
                        document.FlushParts();
                        if (passThru)
