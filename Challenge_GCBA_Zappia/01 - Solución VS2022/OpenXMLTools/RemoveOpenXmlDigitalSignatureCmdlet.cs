...
namespace OpenXml.PowerTools.Commands
{
    ...
    public class RemoveOpenXmlDigitalSignaturesCmdlet : OpenXmlCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                foreach (var document in AllDocuments("Remove-OpenXmlDigitalSignature", FileAccess.ReadWrite))
                {
                    try
                    {
                        document.DigitalSignatures.RemoveAll();
                        document.FlushParts();
                        if (passThru)
