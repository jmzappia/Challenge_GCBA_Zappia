...
namespace OpenXml.PowerTools.Commands
{
    ...
    public class AddOpenXmlDocumentTOACmdlet : OpenXmlCmdlet
    {
        ...
        [Parameter(
            Position = 1,
            Mandatory = false,
            HelpMessage = "Path of the styles file used to get the styles of the TOA")
        ]
        ...
        [Parameter(
            Position = 2,
            Mandatory = false,
            HelpMessage = "Specifies if the styles used in the TOA, must be added to the document")
        ]
        [ValidateNotNullOrEmpty]
        ...
        protected override void ProcessRecord()
        {
            try
            {
                foreach (var document in AllDocuments("Add-OpenXmlDocumentTOA", FileAccess.ReadWrite))
                {
                    try
                    {
                        document.GenerateTOA(Position.Beginning, stylesSourcePath, addDefaultStyles);
                        document.FlushParts();
                        if (passThru)
