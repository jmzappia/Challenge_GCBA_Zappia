...
namespace OpenXml.PowerTools.Commands
{
    ...
    public class AddOpenXmlDocumentTOCCmdlet : OpenXmlCmdlet
    {
        ...
        [Parameter(
            Position = 1,
            Mandatory = false,
            HelpMessage = "Path of the styles file used to get the styles of the TOC")
        ]
        ...
        [Parameter(
            Position = 2,
            Mandatory = false,
            HelpMessage = "Specifies if the styles used in the TOC must be added to the document")
        ]
        [ValidateNotNullOrEmpty]
        ...
        protected override void ProcessRecord()
        {
            try
            {
                foreach (var document in AllDocuments("Add-OpenXmlDocumentTOC", FileAccess.ReadWrite))
                {
                    try
                    {
                        document.GenerateTOC(stylesSourcePath, addDefaultStyles);
                        document.FlushParts();
                        if (passThru)
